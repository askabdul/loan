...
const adminInclude = [
  {
    model: Role,
    as: "Role",
    attributes: ["id", "name", "displayName", "permissions", "hierarchy"],
  },
];
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Auto-generate a unique username from firstName + lastName
      ...
      let attempt = 0;
      while (await Admin.findOne({ where: { username } })) {
        attempt += 1;
...
router.put(
  "/admins/:id",
  requireActionPermission("editAdmin"),
  [
    ...
    try {
      ...
      if (employeeNumber && employeeNumber !== admin.employeeNumber) {
        ...
        if (dup)
          return res.status(400).json({
            success: false,
            message: "Admin with this employee number already exists",
          });
      }
...
router.post(
  "/roles",
  requireActionPermission("manageRoles"),
  [
    body("name").notEmpty(),
  ...
  async (req, res) => {
    try {
      ...
      const { name, displayName, description, hierarchy, permissions } =
        req.body;
  ...
  async (req, res) => {
    try {
      ...
      if (name && name !== role.name) {
        const dup = await Role.findOne({ where: { name } });
        if (dup)
          return res
            .status(400)
            .json({ success: false, message: "Role name already exists" });
      }
      if (hierarchy && hierarchy !== role.hierarchy) {
        const dup = await Role.findOne({ where: { hierarchy } });
        if (dup)
          return res.status(400).json({
            success: false,
...
router.get("/officers", async (req, res) => {
  try {
    ...
    const where = {};
    // status=all means no filter; active/inactive filter by isActive
    if (status !== "all") where.isActive = status === "active";

    let roleWhere = {};
    if (role) {
      // Accept comma-separated role names, e.g. "review-officer,review-lead"
      const roleNames = role.split(",").map((r) => r.trim());
      roleWhere = { name: { [Op.in]: roleNames } };
    }
...
router.get("/permissions", async (req, res) => {
  try {
    ...
    const permissions = {
      menus: effectivePermissions.menus || {},
