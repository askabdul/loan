...
import { FiSearch, FiUpload, FiPlus, FiEye, FiRefreshCw, FiUser, FiCreditCard, FiCheckCircle } from 'react-icons/fi';
...
const OrderRepayment = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      setLoading(true);
      const response = await apiService.get(`/loan-clearance/list?page=${currentPage}&limit=${itemsPerPage}`);
  ...
  const handleLoanSearch = async (e) => {
    ...
    try {
      ...
      const response = await apiService.get(`/loan-clearance/search-loan?userId=${searchForm.userId}&loanId=${searchForm.loanId}`);
      
      if (response.success) {
        setSearchedLoan(response.data.loan);
        setClearanceForm(prev => ({
          ...prev,
          amountCleared: response.data.loan.outstandingBalance || ''
        }));
      } else {
        setError(response.message || 'Loan not found');
  ...
  const handleSubmitClearance = async (e) => {
    ...
    if (clearanceForm.paymentType === 'partial' && !clearanceForm.amountCleared) {
    ...
    try {
      ...
      const response = await apiService.post('/loan-clearance/submit', formData, {
        headers: {
      ...
      if (response.success) {
        ...
        setClearanceForm({ paymentType: 'full', amountCleared: '', popFile: null, notes: '' });
        setSearchedLoan(null);
        // Refresh submissions
        fetchSubmissions();
      } else {
        setError(response.message || 'Failed to submit clearance request');
      }
    } catch (error) {
    ...
    return (
      <div className="p-6 bg-gray-50 min-h-screen w-full flex items-center justify-center">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 text-center max-w-md">
          ...
          <p className="text-sm text-gray-400">You don't have permission to access loan clearance functionality.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 bg-gray-50 min-h-screen w-full">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 m-0 leading-none">Order Repayment</h1>
            <p className="text-xs text-gray-400 mt-0.5">Submit and track loan clearance requests</p>
          </div>
        </div>
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-3 mb-5 text-sm text-red-700 flex items-center justify-between">
          {error}
          <button onClick={() => setError('')} className="text-red-400 hover:text-red-700 ml-4 text-lg leading-none">&times;</button>
        </div>
      )}
      {success && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-3 mb-5 text-sm text-emerald-700 flex items-center justify-between">
          {success}
          <button onClick={() => setSuccess('')} className="text-emerald-400 hover:text-emerald-700 ml-4 text-lg leading-none">&times;</button>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
        ...
        <form onSubmit={handleLoanSearch} className="flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-[160px]">
            ...
            <div className="relative">
              ...
              <input type="text" value={searchForm.userId} onChange={(e) => setSearchForm(prev => ({ ...prev, userId: e.target.value }))} placeholder="Enter User ID" required className={`${inp} pl-9`} />
            </div>
          </div>
          <div className="flex-1 min-w-[160px]">
            ...
            <div className="relative">
              ...
              <input type="text" value={searchForm.loanId} onChange={(e) => setSearchForm(prev => ({ ...prev, loanId: e.target.value }))} placeholder="Enter Loan ID" required className={`${inp} pl-9`} />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-2.5">
              {[['Borrower', searchedLoan.user?.fullName || 'N/A'], ['Loan Amount', formatCurrency(searchedLoan.amount)], ['Outstanding Balance', formatCurrency(searchedLoan.outstandingBalance)], ['Status', searchedLoan.status], ['Due Date', formatDate(searchedLoan.dueDate)]].map(([lbl, val]) => (
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-4">Submit Clearance</p>
            <form onSubmit={handleSubmitClearance} className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Payment Type</label>
                <select value={clearanceForm.paymentType} onChange={(e) => setClearanceForm(prev => ({ ...prev, paymentType: e.target.value, amountCleared: e.target.value === 'full' ? searchedLoan.outstandingBalance : '' }))} className={inp}>
                  <option value="full">Full Payment</option>
                  <option value="partial">Partial Payment</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Amount Cleared</label>
                <input type="number" step="0.01" min="0" max={searchedLoan.outstandingBalance} value={clearanceForm.amountCleared} onChange={(e) => setClearanceForm(prev => ({ ...prev, amountCleared: e.target.value }))} placeholder="Enter amount cleared" disabled={clearanceForm.paymentType === 'full'} required className={inp} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Proof of Payment</label>
                <label htmlFor="popFile" className="flex items-center gap-2 px-4 py-2.5 text-sm border border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-blue-400 bg-gray-50 text-gray-500 hover:text-blue-600 transition">
                  <FiUpload size={13} />
                  {clearanceForm.popFile ? clearanceForm.popFile.name : 'Choose file (JPG, PNG, PDF)'}
                </label>
                <input type="file" id="popFile" accept=".jpg,.jpeg,.png,.pdf" onChange={handleFileUpload} className="hidden" required />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Notes (Optional)</label>
                <textarea value={clearanceForm.notes} onChange={(e) => setClearanceForm(prev => ({ ...prev, notes: e.target.value }))} placeholder="Add any additional notes..." rows={2} className={`${inp} resize-none`} />
              </div>
              <button type="submit" disabled={loading}
                ...
                <FiPlus size={13} /> {loading ? 'Submitting...' : 'Submit Clearance'}
              </button>
            </form>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto mb-5 w-full">
        <div className="px-5 py-3.5 border-b border-gray-100">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest m-0">Recent Submissions</p>
        </div>
        {loading && submissions.length === 0 ? (
          ...
          <table style={{ minWidth: '700px' }} className="w-full">
            ...
            <thead className="bg-gray-50">
              <tr>
                {['Submission ID','User ID','Loan ID','Payment Type','Amount','Status','Submitted','Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-50">
              {submissions.length > 0 ? submissions.map(s => (
                <tr key={s._id} className="hover:bg-gray-50/60 transition">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-600">{s.user?.userId || 'N/A'}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${STATUS_BADGE[s.status] || STATUS_BADGE.pending}`}>{s.status}</span>
                  </td>
                ...
                <tr><td colSpan={8} className="px-4 py-12 text-center text-sm text-gray-400">No submissions found</td></tr>
              )}
            </tbody>
          </table>
        )}
            ...
            selectedSubmission.popUrl && (
              <div className="mt-4">
                ...
                {selectedSubmission.popUrl.endsWith('.pdf') ? (
                  <a href={selectedSubmission.popUrl} target="_blank" rel="noopener noreferrer"
                    className="text-sm text-blue-600 underline">View PDF Document</a>
                ) : (
