...
import { FiPlus, FiEdit, FiTrash2, FiX, FiSearch, FiRefreshCw, FiFileText, FiFilter } from 'react-icons/fi';
...
const ContentManagement = () => {
  ...
  useEffect(() => {
    ...
    websocketService.on('content_deleted', (data) => {
      setContents(prev => prev.filter(content => content._id !== data.contentId));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const contentData = {
        ...formData,
        content: formData.type === 'faq' || formData.type === 'process_guide' || formData.type === 'contact_info' 
          ? (formData.content.startsWith('{') ? JSON.parse(formData.content) : formData.content)
          : formData.content,
      ...
      if (editingContent) {
        ...
        const updatedContent = await apiService.updateContent(editingContent._id, contentData);
        setContents(prev => 
          prev.map(content => 
            content._id === editingContent._id ? updatedContent.content : content
          )
        );
  ...
  const filteredContents = contents.filter(content => {
    ...
    const matchesStatus = statusFilter === 'all' || content.isActive?.toString() === statusFilter;
  ...
  const getTypeColor = (type) => {
    switch (type) {
      ...
      case 'process_guide': return 'primary';
      case 'contact_info': return 'warning';
      case 'terms': return 'secondary';
      case 'privacy': return 'secondary';
      case 'about': return 'success';
    ...
    <div className="p-6 bg-gray-50 min-h-screen w-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 m-0 leading-none">Content Management</h1>
            <p className="text-xs text-gray-400 mt-0.5">Manage app content, FAQs and guides</p>
          </div>
        </div>
      ...
      {/* Message banner */}
      {message.text && (
        <div className={`px-5 py-3 mb-5 rounded-xl text-sm border ${message.type === 'error' ? 'bg-red-50 text-red-700 border-red-200' : message.type === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
          {message.text}
        </div>
      )}
      ...
      {filteredContents.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-8 py-16 text-center text-sm text-gray-400">No content found</div>
      ) : (
        <div className="space-y-3">
          {filteredContents.map(content => (
            <div key={content._id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <h3 className="text-sm font-semibold text-gray-800 m-0">{content.title}</h3>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${content.isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                      {content.isActive ? 'Active' : 'Inactive'}
                    </span>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${TYPE_BADGE[content.type] || 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                      {content.type?.replace('_', ' ').toUpperCase()}
                    </span>
                    {content.key && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono bg-gray-100 text-gray-500 border border-gray-200">{content.key}</span>
                    )}
                  </div>
                  <p className="text-xs text-gray-400 m-0 line-clamp-2">
                    {typeof content.content === 'string' ? content.content.substring(0, 150) : JSON.stringify(content.content).substring(0, 150)}...
                  </p>
                ...
                <div className="flex items-center gap-1">
                  {hasActionPermission('editContent') && (
                    <button onClick={() => handleEdit(content)} title="Edit"
                  ...
                  {hasActionPermission('deleteContent') && (
                    <button onClick={() => handleDelete(content._id)} title="Delete"
      ...
      {showModal && ReactDOM.createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          ...
          <div style={{ position: 'relative', background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-base font-bold text-gray-800 m-0">{editingContent ? 'Edit Content' : 'Create Content'}</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.key} onChange={(e) => handleInputChange('key', e.target.value)} placeholder="e.g., faq_how_to_apply" disabled={!!editingContent} className={inp} />
                  <p className="text-[11px] text-gray-400 mt-1">Unique identifier (cannot change after creation)</p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Type</label>
                  <select value={formData.type} onChange={(e) => handleInputChange('type', e.target.value)} className={inp}>
                    <option value="faq">FAQ</option>
                    <option value="process_guide">Process Guide</option>
                    <option value="contact_info">Contact Info</option>
                    <option value="terms">Terms & Conditions</option>
              ...
              <div>
                ...
                <input type="text" value={formData.title} onChange={(e) => handleInputChange('title', e.target.value)} placeholder="Enter content title" className={inp} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Order</label>
                  <input type="number" value={formData.order} onChange={(e) => handleInputChange('order', parseInt(e.target.value) || 1)} min="1" className={inp} />
                ...
                <div>
                  ...
                  <select value={formData.isActive} onChange={(e) => handleInputChange('isActive', e.target.value === 'true')} className={inp}>
                    <option value={true}>Active</option>
                    <option value={false}>Inactive</option>
                  </select>
                </div>
              ...
              <div>
                ...
                <textarea value={formData.content} onChange={(e) => handleInputChange('content', e.target.value)} placeholder={getContentPlaceholder(formData.type)} rows={8}
                ...
                <p className="text-[11px] text-gray-400 mt-1">{getContentHelp(formData.type)}</p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Metadata (JSON)</label>
                <textarea value={formData.metadata} onChange={(e) => handleInputChange('metadata', e.target.value)} placeholder='{"icon": "📱", "color": "#007bff"}' rows={3}
                  className="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white resize-none font-mono" />
                <p className="text-[11px] text-gray-400 mt-1">Optional additional metadata in JSON format</p>
              </div>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100">
              <button type="button" onClick={() => setShowModal(false)} disabled={saving}
                className="px-5 py-2.5 text-sm font-semibold text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition">Cancel</button>
