...
import { FiSearch, FiFilter, FiEye, FiRefreshCw, FiDollarSign, FiClock, FiCheck, FiX, FiRotateCcw } from 'react-icons/fi';
    ...
    <div className="p-6 bg-gray-50 min-h-screen w-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 m-0 leading-none">Payment Management</h1>
            <p className="text-xs text-gray-400 mt-0.5">Track and manage platform payments</p>
          </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {[
          ...
          <div key={label} className="flex flex-col items-center bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-2 min-w-[72px]">
            <span className="text-base font-bold text-blue-600 leading-tight">{value}</span>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Search by transaction ID, user, or loan..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        <>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto mb-5 w-full">
            <table className="w-full text-sm" style={{ minWidth: '700px' }}>
              <colgroup>
                <col style={{ width: '14%' }} />
                {hasDataAccess('users', 'personalInfo') && <col style={{ width: '12%' }} />}
                {hasDataAccess('loans', 'basic') && <col style={{ width: '10%' }} />}
                {hasDataAccess('payments', 'amount') && <col style={{ width: '10%' }} />}
                <col style={{ width: '8%' }} />
                {hasDataAccess('payments', 'provider') && <col style={{ width: '10%' }} />}
                <col style={{ width: '10%' }} />
                <col style={{ width: '12%' }} />
                <col style={{ width: '8%' }} />
              </colgroup>
              <thead>
                <tr className="bg-gray-50">
                  {['Transaction ID', ...(hasDataAccess('users', 'personalInfo') ? ['User'] : []), ...(hasDataAccess('loans', 'basic') ? ['Loan ID'] : []), ...(hasDataAccess('payments', 'amount') ? ['Amount'] : []), 'Type', ...(hasDataAccess('payments', 'provider') ? ['Provider'] : []), 'Status', 'Date', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {payments.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-12 text-center text-sm text-gray-400">No payments found</td></tr>
                ) : payments.map((payment) => (
                  <tr key={payment._id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3 text-xs font-mono text-gray-700">{payment.transactionId || payment.paymentReference}</td>
                    {hasDataAccess('users', 'personalInfo') && <td className="px-4 py-3 text-sm text-gray-700">{payment.user?.firstName} {payment.user?.lastName}</td>}
                    {hasDataAccess('loans', 'basic') && <td className="px-4 py-3 text-xs font-mono text-gray-500">{payment.loan?._id?.slice(-8) || '—'}</td>}
                    {hasDataAccess('payments', 'amount') && <td className="px-4 py-3 text-sm font-semibold text-gray-800">{formatAmount(payment.amount)}</td>}
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${payment.paymentType === 'full' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                        {payment.paymentType}
                      </span>
                    </td>
                    {hasDataAccess('payments', 'provider') && <td className="px-4 py-3 text-xs text-gray-600">{payment.mobileMoneyProvider}</td>}
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${STATUS_BADGE[payment.status] || 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                        {payment.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">{formatDate(payment.createdAt)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleViewDetails(payment)} title="View Details"
                        ...
                        {payment.status === 'failed' && hasActionPermission('managePayments') && (
                          <button onClick={() => handleRetryPayment(payment._id)} title="Retry Payment"
      ...
      {isModalOpen && selectedPayment && (
        <DetailModal type="payment" data={selectedPayment} onClose={handleCloseModal} />
      )}
    </div>
  );
