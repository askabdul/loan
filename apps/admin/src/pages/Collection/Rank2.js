...
const Rank2 = () => {
  ...
  }, [startDate, endDate, teamFilter, rankingMetric]);
  ...
  const fetchOfficerPerformance = async () => {
    try {
      ...
      const response = await fetch(
        `/api/performance/rankings?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}&type=collection`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
          },
  ...
  const exportData = async (format) => {
    try {
      ...
      a.href = url;
      a.download = `collection-officer-performance-${startDate.toISOString().split("T")[0]}-to-${endDate.toISOString().split("T")[0]}.${format}`;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 m-0 leading-none">
              Collection Officer Performance (Rank2)
            </h1>
            <p className="text-xs text-gray-400 mt-0.5">
              Detailed daily performance metrics for collection officers
            </p>
          </div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {paginatedData.length === 0 ? (
                  <tr>
                    ...
                    >
                      No collection performance data available for the selected
                      criteria
                    </td>
                  </tr>
                ) : (
