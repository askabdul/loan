...
const ForgotPin = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      // Format phone and call backend — backend generates+stores the OTP
      const formattedPhone = phone.trim().startsWith("+")
        ? phone.trim()
        : "+233" + phone.trim().replace(/^0/, "");

      const res = await fetch(
        `${process.env.REACT_APP_API_URL}/auth/reset-pin-request`,
        {
          method: "POST",
      ...
      // Store formatted phone for later steps
      setPhone(formattedPhone);
      setStep("otp");
      toast.success("OTP sent to your phone number");

      // In dev the backend echoes the OTP — show it for easy testing
      if (data.devOtp) {
        toast.info(`[DEV] Your OTP is: ${data.devOtp}`, { autoClose: 15000 });
      }
    } catch (err) {
  ...
  const handleVerifyOtp = (e) => {
    ...
    // OTP will be validated server-side when the PIN is submitted
    setStep("newPin");
  };
  ...
  const handleResetPin = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/auth/reset-pin-verify`,
        {
          method: "POST",
          headers: {
          ...
          <div className="col-12 col-sm-10 col-md-8 col-lg-12 col-xl-12">
            <div className="card cedi-card shadow-lg border-0">
              <div className="card-body p-4 p-md-5">
                ...
                {/* Removed demo OTP display - production app should not show OTP */}
